...
from sentence_utils import is_sentence_done

class SentGenerator(GenerationMixin):
    """
    ...
    def generate(
        ...
        **model_kwargs,
        ...
        self.context = self.tokenizer.decode(inputs_tensor[0], skip_special_tokens=True)
        ...
        while True:
            ...
            if prev_sent_end:
                ...
                for batch_idx in range(input_ids.size(0)):
                    if (len(input_ids[batch_idx])>1 and input_ids[batch_idx][-2] == pad_token_id) or is_sentence_done(self.tokenizer.decode(input_ids[batch_idx][:-1])):
    ...
    def beam_search(
        ...
        **model_kwargs,
        ...
        while True:
            ...
            next_indices = torch_int_div(next_tokens, vocab_size) # get the selected token idx for the beam number
            ...
            if prev_sent_end: # allow at least 1 token produced before the next sentence
                ...
                for batch_idx in range(batch_size):
                    for beam_id in range(num_beams):
                        ...
                        if ((len(input_ids[batch_beam_idx]) > 1) and (input_ids[batch_beam_idx][-2] == pad_token_id)) or is_sentence_done(self.tokenizer.decode(input_ids[batch_beam_idx, :-1])):
