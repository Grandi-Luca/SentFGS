...
from factgraph.fg_utils import process_triples, update_triples, align_triples_tok
import logging
import sys
...
def preprocess_function(examples, tokenizer, num_document_graphs,all_amr_graphs):
    ...
    for sent_id, graph_summary in zip(examples["sent_id"], examples["graph_summary"]):
        ...
        graphs = all_amr_graphs[sent_id]
        for g in graphs[:num_document_graphs]:
            # doc_graph_string_tok = tokenizer.tokenize("[CLS] " + g["graph_dict"]["amr_simple"] + " [SEP]")

            # triples = json.loads(g["graph_dict"]["triples"])
            # # triples = process_triples(triples)
            # all_triples = align_triples(doc_graph_string_tok, "[CLS] " + g["graph_dict"]["amr_simple"] + " [SEP]", triples, 1)
            ...
            enc_graphs.append(g['enc_graphs'])
            enc_graphs_attn.append(g["enc_graphs_attn"])
            mask_graphs.append(1)
            graph_edge_index.append(g["graph_edge_index"])
            graph_edge_type.append(g["graph_edge_type"])
    ...
    assert len(result["graphs"]) == len(result["graphs_attn"]) \
           == len(result["input_ids"]) == len(result["edge_index"]) \
           == len(result["edge_type"]) == len(result["hyp_id"])
